...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	public void draw(PGraphics pg, float x, float y) 
	{
		...
		float delta = this.getRadius() / 2;
		
		if(age.equals("Past Day"))
		{
			if(this.getClass() == LandQuakeMarker.class)
			{
				pg.line(x - delta, y + delta, x + delta, y - delta);
				pg.line(x - delta, y - delta, x + delta, y + delta);
			}
			else
			{
				pg.line(x , y, x + (2*delta), y + (2*delta));
				pg.line(x + (2*delta), y, x, y + (2*delta));
			}
		}
